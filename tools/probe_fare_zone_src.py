...
def dissect_bundle(report):
    ...
    b['pricing_ctx'] = snippets(body, ['pricingZones', 'PricingZones', 'pricing-zones',
                                       'pricingzones', 'PricingZone'], ctx=500, cap=30)
    endpoints = sorted(set(re.findall(r'["\'`]((?:/[\w-]+)*/?api/[\w/-]+)["\'`]', body)))
