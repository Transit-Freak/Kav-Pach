        ...
        for rd2 in set(prev) | set(cur):
            ...
            for b in set(pb) | set(cb):
                ...
                if len(pairs) == 1 and total and len(changed) == total:
                    o, n = next(iter(pairs))
                    # "בכל 1 היציאות" נקרא שבור (בקשת המשתמש): ליציאה
                    # בודדת מנסחים ביחיד, ולכמה — "בכל היציאות ... במקום X"
                    if total == 1:
                        if o == 1:
                            parts = [f'ביציאה היחידה ביום מתוכננים עכשיו {n} אוטובוסים במקום 1 (תגבור)']
                        elif n == 1:
                            parts = [f'התגבור ביציאה היחידה ביום בוטל — אוטובוס אחד במקום {o}']
                        else:
                            parts = [f'ביציאה היחידה ביום — {n} אוטובוסים במקום {o} (תגבור)']
                    elif o == 1:
                        parts = [f'בכל היציאות מתוכננים עכשיו {n} אוטובוסים במקום 1 (תגבור לכל הקו)']
                    elif n == 1:
                        parts = [f'התגבור בוטל בכל היציאות — אוטובוס אחד במקום {o}']
                    else:
                        parts = [f'בכל היציאות — {n} אוטובוסים במקום {o} (תגבור לכל הקו)']
                else:
