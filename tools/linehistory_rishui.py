...
def desc(s, t):
    """תיאור הרכב במילים: גודל + סוג הקו. "אוטובוס" אצל המשרד הוא קטגוריית גודל
    (לא מיניבוס, לא מידיבוס, לא מפרקי) — "אוטובוס רגיל" לא אמר כלום (שלמה 06.09),
    אז: "אוטובוס עירוני בגודל מלא", "מיניבוס עירוני", "אוטובוס מפרקי בינעירוני"."""
    if s == UNDEF or not s:
        return UNDEF
    if s == 'אוטובוס':
        return ' '.join(x for x in ('אוטובוס', t, 'בגודל מלא') if x)
    if s == 'מפרקי':
