...
אזהרה להרחבה ארצית: תנאי השימוש של govmap/מפ"י אוסרים במפורש שימוש
בתכניהם לאימון מודלים ולפיתוח אלגוריתמי בינה מלאכותית בלי אישור בכתב
ומראש. אין למשוך משם תצלומים לפיילוט הזה. ראו docs/govmap-terms.md.

לתל אביב אין איסור מקביל — אין שם שום התייחסות לאימון מודלים, ולשרת
ה-GIS אין מסמך תנאים משלו כלל. המשיכה נשמרת בהיקף קטן וממוקדת מיקומי
תחנות, כדי להישאר בתחום השימוש ההוגן. ראו docs/tlv-terms.md.
"""
import io
import json
import math
