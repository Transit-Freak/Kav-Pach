#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""סימון שינויים שהתבטלו מיד — תנודה בפרסום, לא שינוי במסלול.
...
אותו דבר קורה לשרטוט: בקו 76 קדמה–קרית מלאכי התחלף הציור בין שתי גרסאות
(8,240 מ׳ ו-7,819 מ׳) חמש פעמים בתוך שבועיים, וכל התחלפות נרשמה כ"תיקון
שרטוט" נפרד. ההשוואה כאן היא על רצף התחנות ועל הציור גם יחד, כי אותה
תנודה מופיעה בשניהם.
...
def main():
    ...
    for p in glob.glob(f'{OUTDIR}/lines/*.json'):
        ...
        # החתימה כוללת גם את הציור: תנודה יכולה להיות ברצף, בשרטוט או בשניהם
        sig = [([s[0] for s in v['stops']], v.get('shp') or '') for v in vs]
        ch = False
        for i in range(len(vs) - 2):
            a, b, c = vs[i], vs[i + 1], vs[i + 2]
            if sig[i] != sig[i + 2] or sig[i] == sig[i + 1]:
                continue
            g = gap(b['d'], c['d'])
            if g > DAYS:
                continue
