#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""סימון תחנות שלא נמצאו במסלול של אף קו מהמתועדים אצלנו.

הסורק קורא את stops.txt, שהוא רישום התחנות — ולא רשימת התחנות שבשירות.
חלק מהרשומות שם אינן מופיעות ברצף התחנות של אף קו שמתועד אצלנו. מי שרואה
"תחנה חדשה" מבין שנוספה תחנה שאפשר לחכות בה, וזה לא בהכרח נכון לגביהן.

מה שהסימון אומר הוא בדיוק זה ולא יותר: לא נמצאה במסלול של אף קו מתועד.
אי אפשר לקבוע ממנו שאף קו לא עצר בה — הכיסוי שלנו מוגבל לקווים שיש לנו
את רצף התחנות שלהם.
...
def main():
    ...
    print(f'{mode}: {n_st} תחנות שלא נמצאו במסלול של אף קו מתועד · '
          f'{n_ev} אירועים סומנו · {len(served)} מק"טים נמצאו במסלולים',
          file=sys.stderr)
